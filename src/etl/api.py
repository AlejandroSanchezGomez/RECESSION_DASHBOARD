...
from concurrent.futures import ThreadPoolExecutor, as_completed # provides modules for making concurrent HTTP requests.
from abc import ABC, abstractmethod # provides tools to create interfaces and abstract classes.
from enum import Enum # provides tools to create enum classes.
...
class USTREASURY(API):
    ...
    def request_data(id):
        """Implementation of request_data for the U.S.Treasury repository. In order to maximize efficiency, the HTTP request will be performed through concurrency.
        Returns XML data tree.
        """
        ...
        with ThreadPoolExecutor(max_workers=10) as executor: # start of the request concurrency process with a maximum number of concurrent threads of 10.
